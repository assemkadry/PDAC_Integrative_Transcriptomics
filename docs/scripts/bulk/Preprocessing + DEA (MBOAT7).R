...
# Find median MBOAT7 expression level
MBOAT_expr <- expr_log2["ENSG00000125505", ]
MBOAT7_median <- median(MBOAT_expr, na.rm = TRUE)

# Classifying samples according to median MBOAT7 expression
MBOAT7_group <- ifelse(MBOAT_expr > MBOAT7_median, "High", "Low")

# Adding a new column to the metadata
colData(data)$MBOAT7_GROUP <- MBOAT7_group

head(colData(data))

table(colData(data)$MBOAT7_GROUP)

saveRDS(data, "TCGA_PAAD_Classification_Column_MBOAT7.rds")
...
data_2 <- readRDS("TCGA_PAAD_Classification_Column_MBOAT7.rds")
...
# Extract sample metadata with MBOAT7_GROUP classification
coldata <- colData(data_2)

# Make sure MBOAT7_GROUP is a factor with correct levels
coldata$MBOAT7_GROUP <- factor(coldata$MBOAT7_GROUP, levels = c("Low", "High"))

# Create DESeqDataSet object with no-intercept model
dds <- DESeqDataSetFromMatrix(
  countData = raw_counts,
  colData = coldata,
  design = ~ 0 + MBOAT7_GROUP  
)
...
res <- results(dds, contrast = c("MBOAT7_GROUP", "High", "Low"), alpha = 0.05)
...
write.csv(significant_degs_annot, "MBOAT7_Associated_DEGs_Annotated.csv", row.names = FALSE)
...
EnhancedVolcano(
  ...
  title = 'Volcano Plot: MBOAT7 High vs Low',
...
png("MBOAT7_VolcanoPlot.png", width=1200, height=900, res=150)
...
ma_plot <- ggplot(res_annotated, aes(x = log10(baseMean + 1), y = log2FoldChange, color = Significant)) +
  ...
  labs(
    title = "MA Plot: MBOAT7 High vs Low",
...
# Save the MA plot
ggsave("MBOAT7_MA_plot.png", plot = ma_plot, width = 8, height = 6, dpi = 150)
...
annotation_col <- data.frame(
  MBOAT7_Group = colData(data)$MBOAT7_GROUP
)
...
heatmap_obj <- pheatmap(
  ...
  main = "Top 40 DEGs Heatmap (MBOAT7 High vs Low)"
)

# Save the heatmap to file
save_pheatmap_png <- function(pheatmap_obj, filename, gene_count, width=1200, res=150) {
  height <- max(600, gene_count * 25)  # don't go below 600px
...
# Save to PNG
save_pheatmap_png(heatmap_obj, "MBOAT7_Top40_DEGs_Heatmap.png", gene_count = nrow(heatmap_mat))



# Notes:
# The least amount of loss during conversion was produced using the annotable and ensembldb libraries so we moved forward using them.
